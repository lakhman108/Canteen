...
def get_username(user_id):
    url = f'http://18.232.87.185:8000/api/customusers/{user_id}/'
...
def get_delivery_status(last_order_id):
    ...
    url=f'http://18.232.87.185:8000/api/orders/{last_order_id}/'
    response = requests.get(url)
    if response.status_code == 200:
...
def get_remaining_orderdetails(last_order_id):
    url = f'http://18.232.87.185:8000/api/orders/{last_order_id}/orderdetails/'
    response = requests.get(url)
    if response.status_code == 200:
        data = response.json()
...
def view_orders(request):
    url = 'http://18.232.87.185:8000/api/orders/remainingorders/'
    response = requests.get(url)
...
def change_order_status(order_id):
    ...
    url = f'http://18.232.87.185:8000/api/orders/{order_id}/orderdetails/'
    response = requests.get(url)
    if response.status_code == 200:
        response_data = response.json()
...
def mark_order_completed(request):
        ...
        url = f'http://18.232.87.185:8000/api/orderdetails/{order_detail_id}/'
        data = {
            "order": order_id,
            "qty": qty,
            "isdelivered": True
        ...
        if change_order_status(order_id):
            url = f'http://18.232.87.185:8000/api/orders/{order_id}/'
            data = {
